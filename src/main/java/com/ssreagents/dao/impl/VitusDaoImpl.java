...
public class VitusDaoImpl extends BaseDao implements VitusDao {
    ...
    public int saveVitus(Vitus vitus) {
        String sqlBasic = "insert into t_vitus_basic(uid,brand,product_name,goods_num,batch_num,specifications,price" +
                "goods_time,period_validity,seroytpe,promoter,element,fluorescent,genetic_code,gene_name,gene_size" +
                "resistance_gene,storage,qpcr,silver_stain,literature_used)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

        String sqlExtend = "insert into t_vitus_extend(uid,application,application_object_atrain,bio_organization," +
                "application_way,volume,infection_time,is_achieve_goal,cost_performance,overall_evaluation" +
                "comments ,drawings_show,URL,img,createdate,recorderemail) values()";
...
//        return upadate(sqlExtend, vitus.getUid(), vitus.getApplication(), vitus.getApplication_object_atrain(), vitus.getBio_organization(), vitus.getApplication_way(), vitus.getVolume(),
